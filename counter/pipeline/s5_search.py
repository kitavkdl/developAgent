...
def run_search_and_evaluate(*, claim: dict, claim_id: str, route: str,
    ...
    for q in queries:
        ...
        # max_evidence_per_query 절단 — 코드가 자르는 것이지 LLM 판단이 아님
        for doc in resp.results[:max_eval]:
            if deadline_check():
                degraded_reason = ("JOB_TIMEOUT_SECONDS 초과 — 평가 중단, "
                                   "그 시점까지 평가된 candidate만 반영")
                break
            cand = _evaluate_document(
                claim=claim, doc=doc, log_id=log_id, oai=oai, db=db,
                settings=settings, emitter=emitter, required_match_fields=required,
            ...
            if passes_refuted_gate(cand["applicability_check"], required):
                early_stopped = True
                break
        if early_stopped or degraded_reason:
            break
