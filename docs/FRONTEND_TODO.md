...
- [ ] `apps/web/app/database/` 독립 route 추가
- [ ] `SERVICE_ARCHITECTURE.md` §7의 9개 영속화 테이블을 각각 node로 표현
- [ ] 주요 필드 계약과 public/tenant-private 경계 표시
- [ ] source snapshot·evidence·verdict·answer·trace 관계 edge 표시
- [ ] migration DDL로 오인되지 않도록 논리 계약 범위 명시
- [ ] 좁은 화면에서 카드 목록 기반 읽기 순서 보장
